...
for(i in seq_along(covid$INICIO_SINTOMAS)){
	...
	sum(gencovid, na.rm = T) #Analisando para ver se a soma da probabilidade é 1
...
res_base_diarios_sel <- res_base_diarios_sel %>% 
	lapply(function(x)tail(x,5)) %>%
	lapply(function(x)mutate(x,DEFASAGEM = c("D_5","D_4","D_3","D_2","D_1"))) %>%
